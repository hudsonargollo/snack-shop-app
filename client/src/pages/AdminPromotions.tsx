    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold text-slate-900">Promotional Campaigns</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {isLoading ? (
            ...
            promotions.map((promo) => {
              const isActive =
                new Date(promo.startDate) <= new Date() && new Date() <= new Date(promo.endDate);
              return (
                <Card
                  key={promo.id}
                  className={`p-6 ${isActive ? "border-2 border-green-500 bg-green-50" : ""}`}
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    {isActive && <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-1 rounded">
                      ACTIVE
                    </span>}
                  </div>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-2xl font-bold text-amber-600">
                        {promo.discountType === "percentage"
                          ? `${promo.discountValue}%`
                          : `R$ ${parseFloat(promo.discountValue as any).toFixed(2)}`}
                      </span>
                    </div>
                    <div className="text-xs text-slate-600">
                      <p>Start: {new Date(promo.startDate).toLocaleString()}</p>
                      <p>End: {new Date(promo.endDate).toLocaleString()}</p>
                    </div>
                  </div>
            ...
            <Card className="col-span-full p-12 text-center">
              <p className="text-slate-600 mb-4">No campaigns created yet. Create your first promotion!</p>
